# Receipt Protocol Specification (v2.0)
...
- **PUF Signature Check:** Verify the 64-byte `signature` using **Sat A’s Public Key** (retrieved from the Authority database via Sat A's ID).
- **Result:** If valid, this confirms Sat A—and only Sat A—signed this specific execution receipt.
