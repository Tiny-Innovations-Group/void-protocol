...
| 16.5 | [VOID-139](https://www.notion.so/371d64b77e4081608a3dce4d803aa0c7) | Flat-sat race fix: serial mutex + seller beacon state machine + receipts CWD anchor + half-duplex LBT (CAD) | In progress | P0 | **Bug found during #17 bring-up — blocks #17.** Half-duplex RF collision: the seller's free-running 8 s invoice beacon (`seller.cpp:140`) collides with the in-flight downlink legs (ACK/C/D) on the buyer's single shared radio → `PacketA CRC fail` (`buyer.cpp:153`). Plus a latent unsynchronized bouncer serial-write race (3 threads, no mutex). Fix: serial mutex + seller ADVERTISING/AWAITING_RECEIPT state machine (variant a). |
| 16.6 | [VOID-140](https://app.notion.com/p/37dd64b77e40813bb34adfa161319485) | Visible loop close: seller UNLOCK display + buyer PacketD RX handler | In progress | P0 | **Added v14 — blocks #17 visibility.** Node-185 decision: no separate unlock packet — the signed PacketC authorizes, PacketD delivers; "unlock" is the seller-side display moment at D emit. Buyer currently drops the real PacketD silently (stale pre-VOID-135 passthrough at buyer.cpp:184-188 mislabels PacketC as `PACKET_D:`). Display-only; closes with the #16.5 bench-validation session. |
